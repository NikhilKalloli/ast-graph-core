...
async function parseFileAndExtractFeatures(filePath) {
  ...
  try {
    ...
    // Log query errors specifically if they occur during compilation
...
// --- Similarity Calculation Functions (Unchanged) ---

function jaccardSimilarity(setA, setB) {
  if (!(setA instanceof Set) || !(setB instanceof Set)) {
    // console.warn("Attempted Jaccard on non-Set inputs"); // Be less noisy
    return 0;
  }
...
function calculateOverallSimilarity(featuresA, featuresB) {
    ...
    const weights = { imports: 0.3, declarations: 0.3, calls: 0.15, strings: 0.05, counts: 0.2 };
    const simImports = jaccardSimilarity(featuresA.featureSets.importSources, featuresB.featureSets.importSources);
    const declarationsA = new Set([...featuresA.featureSets.functionNames, ...featuresA.featureSets.classNames, ...featuresA.featureSets.methodNames, ...featuresA.featureSets.interfaceNames, ...featuresA.featureSets.typeAliasNames, ...featuresA.featureSets.enumNames, ...featuresA.featureSets.variableNames]);
    const declarationsB = new Set([...featuresB.featureSets.functionNames, ...featuresB.featureSets.classNames, ...featuresB.featureSets.methodNames, ...featuresB.featureSets.interfaceNames, ...featuresB.featureSets.typeAliasNames, ...featuresB.featureSets.enumNames, ...featuresB.featureSets.variableNames]);
    ...
    let totalScore = (simImports * weights.imports) + (simDeclarations * weights.declarations) + (simCalls * weights.calls) + (simStrings * weights.strings) + (simCounts * weights.counts);
    return totalScore;
}
...
async function processCluster(clusterDataFile, targetClusterId) {
  ...
  for (const filePath of fileList) {
    ...
    if (features) {
      // Convert sets to arrays *before storing* for consistent JSON output
  ...
  console.log(`\nSuccessfully extracted features for ${successfulParses} out of ${fileList.length} files.`);

  // *** SAVE INTERMEDIATE FEATURES ***
  const featuresOutputFilename = `cluster_${targetClusterId}_features.json`;
  if (successfulParses > 0) {
      try {
          await fs.writeFile(featuresOutputFilename, JSON.stringify(allFeatures, null, 2));
          console.log(`Intermediate features saved to ${featuresOutputFilename}`);
      } catch (err) {
          console.error(`Failed to save intermediate features: ${err.message}`);
      }
  } else {
      console.log("No features extracted, skipping feature file save.");
  }
  // ********************************

  if (successfulParses < 2) {
  ...
  // Use the actual list of successfully parsed files
  const parsedFilePaths = Object.keys(allFeatures);

  for (let i = 0; i < parsedFilePaths.length; i++) {
      const fileA = parsedFilePaths[i];
      // No need to check if features exist here, we are iterating over keys of allFeatures

      for (let j = i + 1; j < parsedFilePaths.length; j++) {
          const fileB = parsedFilePaths[j];
          // No need to check features for fileB either

          // Re-convert arrays back to sets for Jaccard calculation
          // It's important this happens *here* right before calculation
  ...
  const similarityOutputFilename = `cluster_${targetClusterId}_similarity_edges.json`;
  try {
    await fs.writeFile(similarityOutputFilename, JSON.stringify(similarityResults, null, 2));
    console.log(`Similarity results saved to ${similarityOutputFilename}`);
  } catch (err) {
