    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          // Ensure consistent forward slashes for paths
...
async function parseFileAndExtractFeatures(filePath) {
  ...
  try {
    ...
    for (const { name: captureType, node } of captures) {
      ...
      // nodeType and nodeCounts population removed
...
function createCsvString(records) {
  // Updated header to reflect the removed columns if necessary
  // In this case, the structure per record remains the same, just fewer records overall
  ...
  const rows = records.map(record => [
    ...
    `"${String(record.feature_value).replace(/"/g, '""')}"`, // Ensure value is stringified
...
async function main() {
  try {
    ...
    if (allCsvRecords.length === 0) {
        console.log("No features extracted, skipping CSV and JSON file generation.");
        return;
    }
    ...
    // Create a structure suitable for JSON (grouping by file might be useful)
    const allJsonFeatures = {};
    tsFiles.forEach(filePath => {
        const fileRecords = allCsvRecords.filter(r => r.filename === filePath);
        if (fileRecords.length > 0) {
            allJsonFeatures[filePath] = {
                featureRecords: fileRecords.map(r => ({ // Exclude filename from inner record
                    feature_type: r.feature_type,
                    feature_value: r.feature_value,
                    combined_feature: r.combined_feature
                }))
            };
        }
    });
    ...
    await fs.writeFile('all_ts_features.json', JSON.stringify({
      ...
      featuresByFile: allJsonFeatures // Store the grouped features
    }, null, 2));
